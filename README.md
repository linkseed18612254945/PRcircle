...
- **共享情报池**：每轮由Agent先基于当前问答与已有情报“规划高质量检索词”（方向+理解+验证意图+隐藏变量），并要求大模型直接生成“可检索的具体关键词”（人物/机构/平台/时间节点/政策条款/话题标签），避免抽象空词；系统仅做轻量去重与质量过滤，规划结果先执行检索并将新证据沉淀到会话级共享池。
...
3. **多会话能力**
   ...
   - 支持 Clear Session：清理当前会话消息并还原输入状态
